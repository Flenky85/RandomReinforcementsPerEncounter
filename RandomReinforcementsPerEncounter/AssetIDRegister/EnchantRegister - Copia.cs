...
namespace RandomReinforcementsPerEncounter
{
    ...
    internal static class EnchantCatalog
    {
        ...
        private static string RootWithHand(string root, WeaponGrip hand)
        {
            return hand switch
            {
                ...
                WeaponGrip.Double => root + ".dbl",
                _ => root + ".one",
            };
        }

        // Nuevo Id con mano: respeta tu patrón original pero añade el sufijo antes del .t{tier}
        private static string Id(string root, int tier, WeaponGrip hand)
        ...
        private static int[] GetDebuffDC(ActivationType activation, WeaponGrip hand)
        {
            if (activation == ActivationType.onlyHit)
                return hand == WeaponGrip.OneHanded ? DebuffDCOnlyHit_1H : DebuffDCOnlyHit_2H;

            return hand == WeaponGrip.OneHanded ? DebuffDCOnlyOnFirstHit_1H : DebuffDCOnlyOnFirstHit_2H;
        }
        ...
        public static void RegisterAll()
        {
            ...
            {
                var dcs = GetDebuffDC(ActivationType.onlyHit, WeaponGrip.OneHanded);
                foreach (var d in DebuffOnlyHitDefs)
                {
                    ...
                    RegisterDebuffTiersFor(
                        tiers,
                        name: d.display,
                        nameRoot: RootWithHand(d.root, WeaponGrip.OneHanded), // evita colisiones de bpName/loc keys
                        description: d.desc,
                        buff: d.buff,
                    ...
                    LootBuckets.AddRootVariant(EnchantType.OnHit, d.root, WeaponGrip.OneHanded, AffixKind.Prefix, 10);
                }
            }

            // Debuffs onlyHit — Two-Handed
            {
                var dcs = GetDebuffDC(ActivationType.onlyHit, WeaponGrip.TwoHanded);
                foreach (var d in DebuffOnlyHitDefs)
                {
                    var tiers = Enumerable.Range(1, 6)
                    ...
                    LootBuckets.AddRootVariant(EnchantType.OnHit, d.root, WeaponGrip.TwoHanded, AffixKind.Prefix, 10);
                }
            }
    ...
    {
        private static string RootWithHand(string root, WeaponGrip hand) => hand switch
        {
            WeaponGrip.OneHanded => $"{root}.one",
            WeaponGrip.TwoHanded => $"{root}.two",
            WeaponGrip.Double => $"{root}.dbl",
